      ...
      .then((label) => {
        if (label !== 100) {
          ...
          wx.onSocketMessage((res) => {
            ...
            if (datas.code == 0) {
              ...
              if (that.Base.getMyData().instinfo.state == '2') {
                ...
                msgop=that.replaceSensitiveWords(msg);
                console.log(msgop)
              } else {
                let msg = datas.data.toString()
                ...
                msgop=that.replaceSensitiveWords(JSON.parse(lastLine).text);
          ...
          wx.onSocketClose((e) => {
            console.log(e)
            that.hideLoadings()
            let prompt = that.Base.getMyData().prompt || msgop;
            ...
            this.ismsg(prompt)
              .then((label) => {
                if (label != 100) {
                  ...
                  that.Base.setMyData({
                    prompt: that.replaceSensitiveWords(msgop)
                  })
                }
